...
class LoginControllerTest < ActionController::TestCase
  ...
  test 'PUT update regenerates api key with REMOTE_USER' do
    request.env['REMOTE_USER'] = 'speedee'
    user = users(:speedee)
    key = user.api_key
    put :update
  ...
  test 'PUT update responds unauthorized with api_token' do
    user = users(:speedee)
    key = user.api_key
  ...
  test 'PUT update responds unauthorized without authentication' do
    put :update
    assert_response 401
  end
