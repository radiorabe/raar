...
class StatusTest < ActiveSupport::TestCase
  ...
  setup do
    @original_home = Rails.application.settings.archive_home
  end

  teardown do
    Rails.application.settings.archive_home = @original_home
  end
  ...
  test 'file_system is false if directory does not exist' do
    Rails.application.settings.archive_home = '/does/not/exist'
